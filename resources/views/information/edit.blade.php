...
@section('title', 'Information - Upload')
@section('content')
<div class="px-5">
            ...
            <tr>
              ...
              <td>
                <form action="{{ route('information.delete', ['id' => $value->id]) }}" method="POST">
                  @csrf
                  @method('DELETE')
                  <button onclick="return confirm('Do you want to delete this Info?')" type="submit" class="bg-red-600 hover:bg-red-800 text-white font-bold py-2 px-3 rounded-full">
                    Delete
                  </button>
                </form>
              </td>
              <td>
                <form action="{{ route('information.update.submit', ['id' =>  $value -> id]) }}" method="GET">
                  @csrf
